...
def navigate_wait(x=0, y=0, z=0, yaw=float('nan'), speed=0.3, frame_id='', auto_arm=False, tolerance=0.1):
    ...
    while not rospy.is_shutdown():
        ...
        if math.sqrt(telem.x ** 2 + telem.y ** 2 + telem.z ** 2) < tolerance:
            break
        rospy.sleep(0.2)
#Функция посадки и ожидание её завершения
def land_wait():
    land()
...
navigate_wait(x=0, y=0, z=0.5, speed=0.6, yaw=float('nan'), frame_id='body', auto_arm=True) #Взлёт на 0.5 метров

rospy.init_node('computer_vision_sample') #Создание ROS-ноды
...
while not flag:
	...
	#Обрабатываем qr
        for barcode in barcodes:
            b_data = barcode.data.decode("utf-8") #Извлекаем данные из qr
            b_type = barcode.type 
	    ...
	    #print(b_data)
            if type(b_data) != type(None):
                flag = b_data #Если распознали qr, выходим из цикла

#Получам данные начала трубопровода              
data = flag.split()
xa, ya = data[0], data[1]
#Запоминаем координаты стартовой позиции
telem = get_telemetry(frame_id='aruco_map')
xold=telem.x
yold=telem.y
zold=telem.z

print(xa,ya) # Выводим данные из qr

endprint+="Navigation area x="+xa+', y='+ya+"\n"+"\n" #Пополняем финальный вывод данными начала трубопровода
        ...
        if cv2.contourArea(cnt) > 100:
            ...
            for cnt in contours0:
                ...
                if cv2.contourArea(cnt)<900 and abs(y_minkvad-ycenter)<20 and abs(x_minkvad-xcenter)<20:
                    telem = get_telemetry(frame_id='aruco_map')
                    #Проверяем что не то же самое повреждение
                    if xprob==-1:
                        pass
                    ...
                    cv2.drawContours(cv_image,[box],0,(180, 105, 255),3) # рисуем прямоугольник (обводим повреждение)
                    defect.publish(bridge.cv2_to_imgmsg(cv_image, 'bgr8')) #Публикуем полученное изображение
                    #ищем пятно
                    red = cv2.inRange(hsv, (0, 90, 90), (23, 255, 255))#цвет повреждения
                    ...
                    S=0
                    #Отправляем повреждение в топик
                    detect.publish(bridge.cv2_to_imgmsg(red, 'mono8'))
                    cv_image1=cv_image
                    if len(contours_red)>0:
                        cnt = contours_red[0]
                        if cv2.contourArea(cnt)>10:
                            #Ищем площадь разлива
                            kof=0.1/w_min #отношение ширины трубопровода в метрах к ширине трубопровода в пикселях
                            S=cv2.contourArea(cnt)*kof*kof #Находим площадь разлива
                            rect = cv2.minAreaRect(cnt)
                            box = cv2.boxPoints(rect) # поиск четырех вершин прямоугольника
                            box = np.int0(box) # округление координат
                            cv2.drawContours(cv_image1,[box],0,(255, 0, 0),3) # рисуем прямоугольник (обводим повреждение)
                            detect.publish(bridge.cv2_to_imgmsg(cv_image1, 'bgr8')) # Публикуем полученное изображение
                    print("defect:",str(telem.x),str(telem.y)) # Выводим координаты повреждения
                    if S!=0:
                        print("oil area:",S) #Выводим площадь разлива
                    endprint+=str(number)+". "+"x="+str(telem.x)+", y="+str(telem.y)+", S="+str(S)+"\n" #Пополняем финальный вывод
                    number+=1
...
print(endprint) #Финальный вывод
